...
function Gallery(props) {
  ...
  const { arrAttractions, lang } = props;

  SwiperCore.use([Navigation, Pagination, Scrollbar, A11y]);
  return (
    <FullScreen handle={handle} className={fsMode ? 'FS' : 'FSNone'}>
      <Swiper
        breakpoints={{
          320: {
            slidesPerView: 1
      ...
      >
        {arrAttractions.map((el, index) => (
          <SwiperSlide onDoubleClick={handle.enter} key={index} className="slider-item">
            <div className="slider-img-wrap">
              <img src={el.photoUrl} alt={el.localizations[lang].name} />
            </div>
            <p key={index} className="slide-title">
              {el.localizations[lang].name}
            </p>
            <p>{el.localizations[lang].description}</p>
          </SwiperSlide>
        ))}
      </Swiper>
    </FullScreen>
